...
class BarCodePainter extends CustomPainter {
  ...
  void _drawBarCode128(Canvas canvas, Size size) {
    List<int> code128 = [
      ...
      print('0x400=' + (0x400).toString()); //100111   64  0  0 8 4 2=78
      print('0x5c6=' + (0x5c6).toString()); //111001   64 32 16 0 0 2=114
      print('0x49e=' + (0x49e).toString()); //011011      32 16 0 4 2=54
      // [45,  "M",   "M", "45", "10111011000"],1024   32 16 0 0 0  =48
      // [46,   "N",  "N", "46", "10111000110"],1478         8 4 0  =12
      // [83, "\x13", "s", "83", "10111100100"],1182 256 128 64 32 0 0 4 0 0  =72
      // 01000111001  64 32 16 0 0 2
      //     1024 0   256 128 64 0 0 0 4 2 0  6 70  198    454  1478
      // 10111000110
      // 101110001100
      // 2048 0   512 256 128  0 0 0 8 4 0 0  12 140 396 908 2956
      // 10111011000
      // 1024 0 256 128 64 0 16 8 0 0 0 = 1496
      // 1478 92 12    5c6
      // 1496 93 13 5d8
    print('M-10111011000');
    print('N-10111000110');
    ...
    for (int i = 0; i < strlen; i++) {
      ...
      for (int j = 0; j < 11; j++) {
        ...
        // print('11*lineWidth+11*i*lineWidth+j*lineWidth=' + (11*lineWidth+11*i*lineWidth+j*lineWidth).toString());
        // print('code128[codeValue]=' + code128[codeValue].toString());
        // print('(0x400 & (code128[codeValue] << j))=' + ((0x400 & (code128[codeValue] << j))).toString());

        Rect rect = new Rect.fromLTWH(11*lineWidth+11*i*lineWidth+j*lineWidth,  0.0, lineWidth, height);
        //   [45, "M", "M", "45", "10111011000"],
        // if (data[i] == 'M' && (j== 0 || j== 2 || j== 3 || j== 4 || j== 6 || j== 7)) {
        //   if (j== 0 || j== 2 || j== 3 || j== 4 || j== 6 || j== 7) {
        ...
        // }
         ((0x400 & (code128[codeValue] << j)) == 0x400) ? painter.color = Colors.black : painter.color = Colors.white;
        // print('code128[codeValue]=' + code128[codeValue].toString());
        // print('(code128[codeValue] << $j)=' + (code128[codeValue] << j).toString());
        // print('(0x400 & (code128[codeValue] << j))=' + (0x400 & (code128[codeValue] << j)).toString());
        // print('((0x400 & (code128[codeValue] << j)) == 0x400)=' + ((0x400 & (code128[codeValue] << j)) == 0x400).toString());
        // ((0x400 & (code128[codeValue] << j)) == 0x400) ? print('1') : print('0');
        canvas.drawRect(rect, painter);
      }
    }
